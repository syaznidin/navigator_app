...
const OrderWaypoints = ({ order, onPress, wrapperStyle, containerStyle, textStyle }) => {
    ...
    const payload = order.getAttribute('payload');
    const navPickup = () => {
        // const wazeUrl = `https://waze.com/ul?q=${extractOriginCoordinates}&navigate=yes`; // your waze URL here
        const googleMapsUrl2 = `https://www.google.com/maps/dir/?api=1&destination=${firstWaypoint.address}`; // your Google Maps URL here

        Linking.canOpenURL(googleMapsUrl2)
            .then((supported) => {
                    return Linking.openURL(googleMapsUrl2);
            })
            .catch((err) => console.error('An error occurred', err));
    };
    const navDel = () => {
        // const wazeUrl = `https://waze.com/ul?q=${extractOriginCoordinates}&navigate=yes`; // your waze URL here
        const googleMapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${lastWaypoint.address}`; // your Google Maps URL here
                            ...
                            {firstWaypoint && (
                                <View style={tailwind('w-full flex-row items-start mb-4')}>
                                    ...
                                    <View style={tailwind('w-4/5')}>
                                        ...
                                        <TouchableOpacity onPress={navPickup} style={tailwind('m-4 p-2 bg-blue-500 rounded')}>
                                            <Text style={tailwind('text-white text-center')}>Navigate Pickup</Text>
                                        </TouchableOpacity>
                                        {firstWaypoint.phone && (
                            ...
                            {lastWaypoint && (
                                <View style={tailwind('w-full flex-row items-end')}>
                                    ...
                                    <View style={tailwind('w-4/5')}>
                                        ...
                                        <TouchableOpacity onPress={navDel} style={tailwind('m-4 p-2 bg-blue-500 rounded')}>
                                            <Text style={tailwind('text-white text-center')}>Navigate Delivery</Text>
                                        </TouchableOpacity>
                                        {lastWaypoint.phone && (
